"""Read-only MCP server for HyperAgents-ADK evidence and artifact tools."""
...
SERVER_NAME = "hyperagents-adk-readonly-mcp"
SERVER_VERSION = "0.1.0"

# Keep MCP read-only. Do not expose run_smoke_self_learning_cycle, Docker launch,
# shell/file-edit tools, or arbitrary generated-code execution.
...
async def list_mcp_tools() -> list[mcp_types.Tool]:
    """List read-only HyperAgents-ADK tools exposed through MCP."""
...
async def call_mcp_tool(name: str, arguments: dict[str, Any]) -> list[mcp_types.Content]:
    """Call one read-only HyperAgents-ADK tool by name."""
